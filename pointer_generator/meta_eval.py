...
class Evaluate(object):
    def __init__(self, model_path):
        ...
        # self.batcher = Batcher(vocab=self.vocab, data_path=config.eval_data_path,
        #                        mode='eval',
        #                        batch_size=config.batch_size, single_pass=True)
        self.vocab = Vocab(os.path.join(config.vocab_cache_dir, config.meta_vocab_file),
                           max_size=config.meta_vocab_size)
        self.batcher = MetaBatcher(num_samples_per_task=config.meta_val_K,
                                   datasets=config.meta_val_datasets,
                                   vocab=self.vocab,
                                   split="validation",
                                   mode='eval')
        time.sleep(15)
