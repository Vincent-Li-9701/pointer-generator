...
class BeamSearch(object):
    ...
    def beam_search(self, batch):
        ...
        while steps < config.max_dec_steps and len(results) < config.beam_size:
            ...
            latest_tokens = [
                t if t < self.vocab.size() else self.vocab.word2id(config.UNK_TOKEN) \
            ...
            s_t = (torch.stack(all_state_h, 0).unsqueeze(0),
            ...
            final_dist, s_t, c_t, attn_dist, p_gen, coverage_t = self.model.decoder(y_t,
                                                                                    s_t,
                                                                                    enc_out,
                                                                                    enc_fea,
                                                                                    enc_padding_mask,
            ...
            for i in range(num_orig_beams):
                ...
                for j in range(
                        config.beam_size * 2):  # for each of the top 2*beam_size hyps:
    ...
    def run(self, num_to_eval, print_result):
        ...
        while cnt < num_to_eval:
            ...
            decoded_words = utils.outputids2words(output_ids, self.vocab,
                                                  (batch.art_oovs[
                                                       0] if config.pointer_gen else None))
